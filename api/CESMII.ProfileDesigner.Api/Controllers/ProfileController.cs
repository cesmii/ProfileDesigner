...
namespace CESMII.ProfileDesigner.Api.Controllers
{
    ...
    public class ProfileController : BaseController<ProfileController>
    {
        ...
        [ProducesResponseType(200, Type = typeof(ResultMessageWithDataModel))]
        public async Task<IActionResult> UploadChunks([FromBody] ImportFileChunk model)
        {
            TryValidateModel(model);
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                string chunkId = model.ChunkId.ToString();
                while (chunkId.Length < 4)  //this is more than adequate - there won't be any file with 9999 many chunks
                {
                    chunkId = $"0{chunkId}";
                }
                string processingPath = Path.Combine(_UPLOAD_FOLDER, LocalUser.ObjectIdAAD, model.ProcessId);
                ...
                {
                    await fs.WriteAsync(model.Data);
                    /*
                    byte[] bytes = new byte[model.Data.Length];
                    int bytesRead = 0;
            ...
            catch (Exception ex)
            {
                return Ok(
                    ...
                    {
                        IsSuccess = false,
                        Message = $"Import failed on chunk {model.ChunkId} of {model.TotalChunks}...{ex.Message}",
                        Data = null
                    }
                );
            }
            return Ok(
                ...
                {
                    IsSuccess = true,
                    Message = $"Uploaded chunk {model.ChunkId} of {model.TotalChunks}",
                    Data = model.ChunkId
                }
            );
        }
        ...
        [ProducesResponseType(200, Type = typeof(ResultMessageWithDataModel))]
        public IActionResult UploadComplete([FromBody] ImportFileChunkComplete model)
        {
            ...
            try
            {
                ...
                if (filePaths.Length == 0 || filePaths.Length != model.TotalChunks)
                {
                    return Ok(
                        ...
                        {
                            IsSuccess = false,
                            Message = $"Number of chunked files does not match for {fileName}. Expected: {model.TotalChunks}, actual: {filePaths.Length}",
                            Data = null
                        }
                    );
                }
                ...
                if (content.Length == 0 || content.Length != model.TotalBytes)
                {
                    return Ok(
                        ...
                        {
                            IsSuccess = false,
                            Message = $"Merged file size does not match original file size for {fileName}. Expected: {model.TotalBytes}, actual: {content.Length}",
                            Data = null
                        }
                    );
                }
                ...
                //TODO: now start the import processing
            ...
            catch (Exception ex)
            {
                return Ok(
                    ...
                    {
                        IsSuccess = false,
                        Message = $"Upload complete failed for file {fileName}...{ex.Message}",
                        Data = null
                    }
                );
            }
            return Ok(
                ...
                {
                    IsSuccess = true,
                    Message = $"Upload completed for file {fileName}",
                    Data = null
                }
            );
        }
