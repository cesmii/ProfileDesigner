...
function ProfileTypeDefinitionRow(props) { //props are item, showActions
    ...
    const downloadProfileAsThinkIq= async () => {
        console.log(generateLogMessageString(`downloadProfileThinkIq||start`, CLASS_NAME));
        //add a row to download messages and this will kick off download
        var msgs = loadingProps.downloadItems || [];
        msgs.push({ profileId: props.item.profile?.id, fileName: cleanFileName(props.item.profile?.namespace), immediateDownload: true, downloadFormat: AppSettings.ExportFormatEnum.ThinkIQ });
        setLoadingProps({ downloadItems: JSON.parse(JSON.stringify(msgs)) });
    }
            ...
            <>
                ...
                <Dropdown className="action-menu icon-dropdown ml-2" onClick={(e) => e.stopPropagation()} >
                    ...
                    <Dropdown.Menu>
                        ...
                        <Dropdown.Item key="moreVert6" onClick={downloadProfileAsThinkIq} ><span className="mr-3" alt="arrow-drop-down"><SVGDownloadIcon name="downloadNodeset" /></span>Download Profile '{getProfileCaption(props.item.profile)}' for SMIP import (experimental)</Dropdown.Item>
                    </Dropdown.Menu>
                </Dropdown>
            </>
        );
