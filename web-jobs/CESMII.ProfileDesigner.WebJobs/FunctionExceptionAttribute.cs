...
namespace CESMII.ProfileDesigner.WebJobs.Filters
{
    /// <summary>
    /// Sample exception filter that shows how declarative error handling logic
    /// can be integrated into the execution pipeline.
    /// </summary>
    public class FunctionErrorHandlerAttribute : FunctionExceptionFilterAttribute
    {
        public override Task OnExceptionAsync(FunctionExceptionContext exceptionContext, CancellationToken cancellationToken)
        {
            ...
            exceptionContext.Logger.LogError(exceptionContext.Exception, exceptionContext.Exception.Message);
            Console.Error.WriteLine(exceptionContext.Exception.Message);
